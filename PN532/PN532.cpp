...
*/
/**************************************************************************/
bool PN532::readPassiveTargetID(uint8_t cardtype, uint8_t *uid, uint8_t *uidLength, uint16_t timeout)
{
    pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
    pn532_packetbuffer[1] = 1;  // max 1 cards at once (we can set this to 2 later)
    pn532_packetbuffer[2] = cardtype;
...
/////////////////////////////////////////////////////////////////////////////
uint32_t PN532::readPassiveTargetID(uint8_t cardtype, uint16_t timeout) 
{
  uint32_t cid;          // TT should be 64 for 8 byte id

  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;  // TT 0x4A datasheet p115
  pn532_packetbuffer[1] = 1;  // max 1 cards at once 
  pn532_packetbuffer[2] = cardtype;

  switch (cardtype) {

  case 0x00:                // TT mifare, original
  {
  ...
  Serial.println("baudrate 106 typeA");
  Serial.print("Found "); 
  ...
  Serial.println(pn532_packetbuffer[12], DEC); 
  cid = 0;
...
//  Serial.println(cid);  // TT normal print does not support 64-bit numbers
  ...
  Serial.print(pn532_packetbuffer[27], HEX);
  Serial.print(" ");
  Serial.println(pn532_packetbuffer[28], HEX);
  
  return cid;
  }
  break;

  case 0x02:   // felica 424
  {
  pn532_packetbuffer[3] = 0x00;             // TT datasheet example p121
  pn532_packetbuffer[4] = 0xFF;
  pn532_packetbuffer[5] = 0xFF;
  ...
  Serial.println("baudrate 424 felica");
  Serial.print("Number of targets: ");
  Serial.println(pn532_packetbuffer[7], HEX);
  Serial.print("Logical number: ");
  Serial.println(pn532_packetbuffer[8], HEX);
...
/***** inAutoPoll Functions agregado por john ******/
...
bool PN532::inAutoPoll(uint16_t timeout) // TT added
{

  pn532_packetbuffer[0] = 0x60;  // TT 0x60 datasheet p144
...
/*
  ...
  Serial.println();
*/
  int cardtype = pn532_packetbuffer[8];
  Serial.print("Card type: 0x");
  Serial.print(cardtype, HEX);
  switch (cardtype) {             // see datasheet p144
  case 0x10:
  Serial.print(" =Mifare  IDlength= ");
  Serial.print(pn532_packetbuffer[14], DEC); 
  Serial.print("  ID= ");  
  for(uint8_t i=15;i<15+pn532_packetbuffer[14];i++)
  {
    Serial.print(pn532_packetbuffer[i], HEX); 
    Serial.print(" ");
  }
...
bool PN532::readFeliCa(uint16_t timeout) // TT added
{

/*
  pn532_packetbuffer[0] = 0x40;  // TT 0x40 datasheet p127
  pn532_packetbuffer[1] = 0x01;  // target number
  pn532_packetbuffer[2] = 0x0D;  // length
  pn532_packetbuffer[3] = 0x02;  // cmd (request service)
  pn532_packetbuffer[4] = 0x01;  // ID
  pn532_packetbuffer[5] = 0x01;  // 
  pn532_packetbuffer[6] = 0x07;  // 
  pn532_packetbuffer[7] = 0x01;  //
  pn532_packetbuffer[8] = 0xAF;  //
  pn532_packetbuffer[9] = 0x11;  //
  pn532_packetbuffer[10] = 0x11;  //
  pn532_packetbuffer[11] = 0x2C;  // 
  pn532_packetbuffer[12] = 0x01;  // node 
  pn532_packetbuffer[13] = 0xFF;  // node code list
  pn532_packetbuffer[14] = 0xFF;  // node code list
...
*/
  ...
  pn532_packetbuffer[0] = 0x40;  // TT 0x40 datasheet p127
  pn532_packetbuffer[1] = 0x01;  // target number
  pn532_packetbuffer[2] = 0x0F;  // length
  pn532_packetbuffer[3] = 0x06;  // cmd (read without encryption)
  pn532_packetbuffer[4] = 0x01;  // ID
  pn532_packetbuffer[5] = 0x01;  // 
  pn532_packetbuffer[6] = 0x07;  // 
  pn532_packetbuffer[7] = 0x01;  //
  ...
  pn532_packetbuffer[10] = 0x11;  //
  pn532_packetbuffer[11] = 0x2C;  // 
  pn532_packetbuffer[12] = 0x01;  // number of service 
  pn532_packetbuffer[13] = 0x07;  // service code list
  pn532_packetbuffer[14] = 0x00;  // 
  pn532_packetbuffer[15] = 0x01;  // number of block
  pn532_packetbuffer[16] = 0x11;  // block list


    if (HAL(writeCommand)(pn532_packetbuffer, 17)) {
            return false;  // command failed
    }
